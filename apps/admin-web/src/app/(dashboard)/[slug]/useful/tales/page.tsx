...
import { BookIcon, PencilIcon, TrashIcon } from "@/components/ui/icons";
...
export default function TalesPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [modal, setModal] = useState<{ open: boolean; tale: Tale | null }>({ open: false, tale: null });
  const [deleting, setDeleting] = useState<Tale | null>(null);
  ...
  const talesQuery = useQuery({
    queryKey: ["useful-tales", slug, forcedBranchId, groupFilter],
    queryFn: () => {
      const qs = new URLSearchParams();
      if (groupFilter) qs.set("groupId", groupFilter);
      if (forcedBranchId) qs.set("branchId", forcedBranchId);
      const query = qs.toString();
      return api.get<Tale[]>(`/app/useful/tales${query ? `?${query}` : ""}`);
    },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/app/useful/tales/${id}`),
    onSuccess: () => {
    ...
    <div className="space-y-5">
      ...
      {talesQuery.isLoading ? (
        ...
        <div className="space-y-3">
          {talesQuery.data.map((tale) => (
            <TaleRow
              key={tale.id}
              tale={tale}
              canEdit={canWrite && (!isTeacher || tale.createdById === user?.id)}
              onEdit={() => setModal({ open: true, tale })}
              onDelete={() => setDeleting(tale)}
            />
          ))}
        </div>
      )}
